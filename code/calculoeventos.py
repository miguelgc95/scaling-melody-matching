...
class Event():
    def __init__(self):
        self.eps=[] #esta lista tendra tantos elementos como eventos tenga dicha nota de Q con R. cada uno de estos elementos sera otra lista que guarda 
                    ...
                    #4 alturas de los 6 rectangulos involucrados. el último elemento es el nº del evento del que se trata
        self.index=0
        self.pitch=0
...
def get_epsilons(epsilon, notaactual,notasiguiente, R, maxeps,j):
    ...
    for i in range(len(R)):
        ...
        if R[i][1]>notaactual[1] and e<maxeps:            
            if j==0:
                notaactualdesplazada = (0, notaactual[1]+e*(j+1), notaactual[2]) 
                notasiguientedesplazada = (notasiguiente[0]+e*(j+1), notasiguiente[1] + e*(j+2), notasiguiente[2])                
            else:
                notaactualdesplazada = (notaactual[0]+e*(j), notaactual[1]+e*(j+1), notaactual[2]) 
                notasiguientedesplazada = (notasiguiente[0]+e*(j+1), notasiguiente[1] + e*(j+2), notasiguiente[2])
