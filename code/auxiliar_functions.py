...
def generate_melodies():
    ...
    for i in range(1,15):#R tendra 5 notas(de momento, por tener algo)
        ...
        newpitch=random.randint(100,1000)#los valores posibles en frecuencia
...
def prepare_melody(x):
    ...
    le=len(x)
    for i in range(le):
        if i>0 and abs(S[-1][1]-x.iloc[i,0])>0.001:#este if es para contemplar los silencios como una nota de pitch 0
            s=[]
            s.append(S[-1][1])#por eso se le da el final de la nota anterior como inicio de la nueva nota
            s.append(x.iloc[i,0])#queremos que dure hasta que se empieza a cantar de nuevo
