...
def actualizar(h,q,areainicial,h11,h22,h33,maxeps):
    ...
    ayuda=[]
    while(h):
        recupera=heapq.heappop(h)#recupera es una lista de 3 elementos. el 0 es un valor de epsilon, el 1 es la nota de Q para la que se produce dicho epsilon y el 2 el nº de evento para esa nota
        nuevoepsilon=recupera[0]
        j=recupera[1]
        n=recupera[2]
        nuevoevento=q[j].eps[n]#en esta lista hemos guardado toda la info relativa al proximo evento que se producira en el tiempo(osea, el menos evento siguiente)
        ...
        if len(q[j].eps)>(n+1):#si aun quedan eventos para el segmento del que acabamos de sacar nuevoevento:
        ...
        if nuevoevento[1]==0:
            ...
            h22=h22-(j+1)*nuevoevento[3]-(j+2)*nuevoevento[6]+(j+1)*nuevoevento[5]
            ...
            h22=h22+(j+1)*nuevoevento[5]
            h33=h33-(j+1)*nuevoevento[4]+(j)*nuevoevento[3]+(j+1)*nuevoevento[6]
